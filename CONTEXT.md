...
- **Validation pipeline** — `ValidationPipeline.RunAsync`, the single home of the "run every stage in order, aggregate all errors, never short-circuit" invariant. One generic runner serves both hierarchies because `ValidationRun<TContext>` derives from `ValidationRun`. Each object/collection/dictionary schema declares its own **stage order** as a memoized `Stages()` array — the order is data, not an implicit call sequence (object runs rules last; collection and dictionary run them first). The null-guard and `Result` wrapping stay in each schema's `ValidateAsync` because they are the only genuinely type-specific parts.
