...
public partial class ObjectContextSchema<T, TContext> : ContextSchema<T, TContext> where T : class
{
    ...
    public ObjectContextSchema<T, TContext> If(
        Func<T, TContext, bool> condition,
        Action<ConditionalBuilder<T, TContext>> configure)
    {
        var builder = new ConditionalBuilder<T, TContext>();
        configure(builder);
        _conditionals.Add(new ContextAwareConditionalBranch<T, TContext>(condition, builder, null));
        return this;
    }
    ...
    public ObjectContextSchema<T, TContext> Switch(Action<ContextSwitchBuilder<T, TContext>> configure)
    {
        var builder = new ContextSwitchBuilder<T, TContext>();
        configure(builder);
        _conditionals.Add(new ContextSwitchBranch<T, TContext>(builder.Cases, builder.DefaultSchema));
        return this;
    }
    ...
    /// </summary>
    [Obsolete("Use If() instead. When() will be removed in a future version.")]
