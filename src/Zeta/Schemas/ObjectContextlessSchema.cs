...
public sealed partial class ObjectContextlessSchema<T> : ContextlessSchema<T> where T : class
{
    ...
    public ObjectContextlessSchema<T> If(
        Func<T, bool> condition,
        Action<ContextlessConditionalBuilder<T>> configure)
    {
        var builder = new ContextlessConditionalBuilder<T>();
        configure(builder);
        _conditionals.Add(new ContextlessConditionalBranch<T>(condition, builder, null));
        return this;
    }

    public ObjectContextlessSchema<T> Switch(Action<SwitchBuilder<T>> configure)
    {
        var builder = new SwitchBuilder<T>();
        configure(builder);
        _conditionals.Add(new SwitchBranch<T>(builder.Cases, builder.DefaultSchema));
        return this;
    }
